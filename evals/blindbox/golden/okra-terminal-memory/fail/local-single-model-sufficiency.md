# Local Single Model Sufficiency
...
The reset prep is `memory/continuation-packet.v1.json`. It is candidate only and not authority. The
next run must re-check freshness, context fit, current-run evidence, and human ratification.

The retention proof is `memory/trace-manifest.v1.jsonl`. Each line has sha256/content hash, excerpt
hash, causal decision path, sensitivity, redaction status, retention tier, expiry/tombstone, and the
memory records that depend on that trace.
...
Later, one model report is sufficient proof for memory promotion.
