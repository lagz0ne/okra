...
This repository dogfoods the skill. For non-trivial work on the skill, runner, evals, review
harness, release workflow, or OKRA memory, use `reverse-tornado-okr` as the default workflow. The
default trust anti-goal is `single_llm_truth_acceptance_count == 0`; judgement-heavy claims need
deterministic evidence where possible plus independent Codex/Claude or human review before they are
accepted.
...
For project-default dogfooding behavior, run the natural prompt case that does not inject skill
fallback context:
